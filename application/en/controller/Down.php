...
use app\en\model\Down as DownModel;
...
class Down extends Base{
    ...
    public function view(){
	    ...
	    $pre = !$previous ? lang('No data') : "<a href=\"".url('/'.$previous['urlroute'].'-d'.$previous['id'])."\">".$previous['title']."</a>";
	    $this->assign('pre',$pre);
	    
	    //下一篇
	    $next=Db::name('down')->where('id','>',$id)->where('lang','=',$lang)->where('sortid','in',$sortidArr)->order('sequence desc,id desc')->limit(1)->find();
	    $next = !$next ? lang('No data') : "<a href=\"".url('/'.$next['urlroute'].'-d'.$next['id'])."\">".$next['title']."</a>";
