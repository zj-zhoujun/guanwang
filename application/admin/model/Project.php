...
class Project extends Model{
	...
	static function index(){
		...
		//视图查询
		$rs = Db::view('project','id,sortid,areaid,lang,sequence,title,seotitle,keyword,description,content1,content2,content3,urlroute')
            ...
            ->where($where)->where('project.title|project.seotitle|project.keyword|project.description|project.content1|sort.title|sort.seotitle|sort.keyword|sort.description|area.title','like',"%$keyword%")->order($order)
	...
	static function edit($id){
	    if($id>0){//编辑
			...
			$result = $project->allowField(true)->save(input('post.'),['id'=>$id]);
			...
			//变更类别及对应的属性
			$sortidRs = Db::name('Project')->find($id);
			...
			$project = new Project;
			$result = $project->allowField(true)->save(input('post.'));
		}
		...
		if($arrayNum>0){
		    for($i=1;$i<$arrayNum+1;$i++){
		        ...
		        if(empty($id) || empty($attValueId)){//新增
					$attributeValue->data(['attvalue'=>$data['attvalue'],'attid'=>input("attid".$i),'moduleid'=>4,'infoid'=>$project->id])->save();
				}else{//更新
					$attributeValue->save(['attvalue'=>$data['attvalue'],'attid'=>input("attid".$i),'moduleid'=>4,'infoid'=>$project->id],['id'=>$attValueId]);
				}
		    }
		}
		//返回控制器
