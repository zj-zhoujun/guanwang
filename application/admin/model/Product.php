...
class Product extends Model{
	...
	static function index(){
		...
		//视图查询
		$rs = Db::view('product','id,sortid,areaid,lang,sequence,thumb,title,seotitle,keyword,description,content1,content2,content3,urlroute')
            ->view('sort',['title'=>'s_title','seotitle'=>'s_seotitle','keyword'=>'s_keyword','description'=>'s_description'],'product.sortid=sort.id')
            ->view('area',['title'=>'a_title'],'product.areaid=area.id')
            ->where($where)->where('product.title|product.seotitle|product.keyword|product.description|product.content1|sort.title|sort.seotitle|sort.keyword|sort.description|area.title','like',"%$keyword%")->order($order)
	...
	static function edit($id){
	    if($id>0){//编辑
			...
			$result = $product->allowField(true)->save(input('post.'),['id'=>$id]);
			...
			//变更类别及对应的属性
			$sortidRs = Db::name('Product')->find($id);
			...
			$product = new Product;
			$result = $product->allowField(true)->save(input('post.'));
		}
		...
		if($arrayNum>0){
		    for($i=1;$i<$arrayNum+1;$i++){
		        ...
		        if(empty($id) || empty($attValueId)){//新增
					$attributeValue->data(['attvalue'=>$data['attvalue'],'attid'=>input("attid".$i),'moduleid'=>1,'infoid'=>$product->id])->save();
				}else{//更新
					$attributeValue->save(['attvalue'=>$data['attvalue'],'attid'=>input("attid".$i),'moduleid'=>1,'infoid'=>$product->id],['id'=>$attValueId]);
				}
		    }
		}
		//返回控制器
