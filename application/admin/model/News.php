...
class News extends Model{
	...
	static function index(){
		...
		//视图查询
		$rs = Db::view('news','id,sortid,areaid,lang,sequence,title,seotitle,keyword,description,content1,urlroute')
            ->view('sort',['title'=>'s_title','seotitle'=>'s_seotitle','keyword'=>'s_keyword','description'=>'s_description'],'news.sortid=sort.id')
            ->view('area',['title'=>'a_title'],'news.areaid=area.id')
            ->where($where)->where('news.title|news.seotitle|news.keyword|news.description|news.content1|news.author|news.copyfrom|sort.title|sort.seotitle|sort.keyword|sort.description|area.title','like',"%$keyword%")->order($order)
	...
	static function edit($id){
	    if($id>0){//编辑
			$news = News::get($id);
			$result = $news->allowField(true)->save(input('post.'),['id'=>$id]);
			...
			$news = new News;
			$result = $news->allowField(true)->save(input('post.'));
		}
		...
		if($arrayNum>0){
		    for($i=1;$i<$arrayNum+1;$i++){
		        ...
		        if(empty($id) || empty($attValueId)){//新增
					$attributeValue->data(['attvalue'=>$data['attvalue'],'attid'=>input("attid".$i),'moduleid'=>2,'infoid'=>$news->id])->save();
				}else{//更新
					$attributeValue->save(['attvalue'=>$data['attvalue'],'attid'=>input("attid".$i),'moduleid'=>2,'infoid'=>$news->id],['id'=>$attValueId]);
				}
		    }
		}
		//返回控制器
